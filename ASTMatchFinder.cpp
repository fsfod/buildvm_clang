...
public:
  ...
  MatchASTVisitor()
     : MatcherCallbackPairs(new std::vector<std::pair<const internal::DynTypedMatcher*, MatchCallback*> >()),
     ActiveASTContext(NULL) {
  }
...
void MatchASTConsumer::addMatcher(const DeclarationMatcher &NodeMatch, MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
    new internal::Matcher<Decl>(NodeMatch), Action));
}

void MatchASTConsumer::addMatcher(const TypeMatcher &NodeMatch, MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
    new internal::Matcher<QualType>(NodeMatch), Action));
}

void MatchASTConsumer::addMatcher(const StatementMatcher &NodeMatch,
                             MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
    new internal::Matcher<Stmt>(NodeMatch), Action));
}

void MatchASTConsumer::addMatcher(const NestedNameSpecifierMatcher &NodeMatch, MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
    new NestedNameSpecifierMatcher(NodeMatch), Action));
}

void MatchASTConsumer::addMatcher(const NestedNameSpecifierLocMatcher &NodeMatch, MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
    new NestedNameSpecifierLocMatcher(NodeMatch), Action));
}

void MatchASTConsumer::addMatcher(const TypeLocMatcher &NodeMatch, MatchFinder::MatchCallback *Action) {
  MatcherCallbackPairs.push_back(std::make_pair(
...
using clang::ast_matchers::internal::MatchASTVisitor;

MatchASTConsumer::MatchASTConsumer(clang::ASTContext& astContext) :
  ActiveASTContext(astContext){

  Visitor = new MatchASTVisitor(&MatcherCallbackPairs);

  auto visitor = reinterpret_cast<MatchASTVisitor*>(Visitor);

 // visitor->MatcherCallbackPairs
}

MatchASTConsumer::~MatchASTConsumer(){
  auto visitor = reinterpret_cast<MatchASTVisitor*>(Visitor);
  delete visitor;
}

void MatchASTConsumer::HandleTranslationUnit(ASTContext &Context) {
  auto visitor = reinterpret_cast<MatchASTVisitor*>(Visitor);
...
bool MatchASTConsumer::HandleTopLevelDecl(clang::DeclGroupRef d){
  ...
  visitor->set_active_ast_context(&ActiveASTContext);
